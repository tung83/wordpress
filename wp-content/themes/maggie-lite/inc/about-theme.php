...
class Maggie_Lite_About_Theme {
	public function __construct() {
		...
		$this->maggie_lite_req_plugins = array(
			'instant-demo-importer' => array(
				'slug' => 'instant-demo-importer',
				'name' => __('Instant Demo Importer', 'maggie-lite'),
				...
				'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'maggie-lite'),
				),
			);

		/** Plugin Installation Ajax **/
		add_action( 'wp_ajax_maggie_lite_plugin_installer', array( $this, 'maggie_lite_plugin_installer_callback' ) );

		/** Plugin Installation Ajax **/
		add_action( 'wp_ajax_maggie_lite_plugin_offline_installer', array( $this, 'maggie_lite_plugin_offline_installer_callback' ) );

		/** Plugin Activation Ajax **/
		add_action( 'wp_ajax_maggie_lite_plugin_activation', array( $this, 'maggie_lite_plugin_activation_callback' ) );

		/** Plugin Activation Ajax (Offline) **/
		add_action( 'wp_ajax_maggie_lite_plugin_offline_activation', array( $this, 'maggie_lite_plugin_offline_activation_callback' ) );
		...
		<div class="wrap maggie-lite-wrap">
			...
			<div class="bottom-block">
				<ul class="maggie-lite-tab-wrapper wp-clearfix">
					...
					<li><a href="<?php echo esc_url( admin_url( 'themes.php?page=maggie-lite-about&tab=demo_import' ) ); ?>"
						class="maggie-lite-tab <?php echo $active_tab == 'demo_import' ? 'maggie-lite-tab-active' : ''; ?> "><?php echo esc_html__( 'Import Demo', 'maggie-lite' ); ?>
	...
	public function get_local_dir_path($plugin) {
			...
			$url = wp_nonce_url(admin_url('themes.php?page=maggie-lite-about&tab=demo_import'),'bloog-file-installation');
			if (false === ($creds = request_filesystem_credentials($url, '', false, false, null) ) ) {
				...
				$file_location = get_stylesheet_directory().'/inc/admin-panel/demo/'.$plugin['slug'].'.zip';
			...
			/* ========== Plugin Installation Ajax =========== */
			public function maggie_lite_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'maggie-lite' ) );
				...
				if (! wp_verify_nonce( $nonce, 'maggie_lite_plugin_installer_nonce' ))
					wp_die( __( 'Error - unable to verify nonce, please try again.', 'maggie-lite') );
				...
				// Get Plugin Info
				$api = $this->maggie_lite_call_plugin_api($plugin);
			...
			public function maggie_lite_plugin_offline_installer_callback() {
			...
			public function maggie_lite_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function maggie_lite_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'maggie-lite' ) );

				$nonce = $_POST["nonce"];
				$plugin = $_POST["plugin"];

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'maggie_lite_plugin_activate_nonce' ))
					die( __( 'Error - unable to verify nonce, please try again.', 'maggie-lite' ) );
				...
				// Get Plugin Info
				$api = $this->maggie_lite_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'maggie-lite');
				}
	...
	/** Initializing Demo Importer if exists **/
		...
		$demoimporter->demos = array(
			'maggie-lite' => array(
				'title' => __('Maggie Lite Demo', 'maggie-lite'),
				'name' => 'maggie-lite',
