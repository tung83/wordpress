...
add_thickbox();
$maggie_lite_recommended_plugins = array(
	...
	<div class="feature-section recommended-plugins">
		<?php foreach ( $maggie_lite_recommended_plugins as $plugin => $prop ) { ?>
		<?php
		...
		switch ( $active['needs'] ) {
			...
			$label = __( 'Install', 'maggie-lite' );
			break;
			case 'activate':
			$class = 'activate-now button button-primary';
			$label = __( 'Activate', 'maggie-lite' );
			break;
			case 'deactivate':
			$class = 'deactivate-now button';
			$label = __( 'Deactivate', 'maggie-lite' );
			break;
		}
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ): ?>
				<span class="recommended"><?php _e( 'Required', 'maggie-lite' ); ?></span>
			<?php endif; ?>
			...
			<div class="b-wrap">
			<span class="version"><?php echo __( 'Version:', 'maggie-lite' ); ?><?php echo esc_html( $info->version ) ?></span>
